...
# --- STRICT PATH MATCHING (Segregated for Serverless Tracking) ---
DATASET_DIR = "data/raw/Logistics-2/test" 

# Log targets explicitly tracking your Fireworks AI infrastructure run
LEDGER_JSON_PATH = "compliance_time_series_ledger_fireworks.json"
LEDGER_CSV_PATH = "compliance_alerts_log_fireworks.csv"
...
# Connect to the Fireworks Gateway via the unified OpenAI client
client = OpenAI(
...
def init_ledgers():
    """Safely checks for existing diagnostics logs and initializes arrays/headers if missing."""
...
def analyze_warehouse_frame_via_fireworks(image_path):
    """Dispatches payload to your on-demand deployment endpoint on Fireworks and profiles speed."""
    try:
        ...
        # Clean markdown wrappers if they bypass the vision model parameters
        ...
        # Approximate tokens based on text splitting length
        approx_tokens_generated = len(output_text.split()) * 1.3
        throughput = approx_tokens_generated / latency
        
        return parsed_json, latency, throughput

    except Exception as e:
        return {"error": f"Cloud deployment response error: {str(e)}"}, 0.0, 0.0
...
def run_pure_fireworks_pipeline():
    ...
    for idx, filename in enumerate(image_files, start=1):
        ...
        payload, latency, throughput = analyze_warehouse_frame_via_fireworks(image_path)
        ...
        # Keep track of active performance parameters
        telemetry_logs.append({
            "Frame": filename,
            "Latency (s)": latency,
            "Throughput (tk/s)": throughput
        })
        ...
        print(f"  ⚡ Telemetry -> Latency: {latency:.2f}s | Speed: {throughput:.1f} tk/s")
        print(f"  📊 Metrics   -> Items: {total_items_counted} | Breaches: {len(valid_violations)} (🔥 {high_risk_count} CRITICAL)")
        print("-" * 80)

    # --- FINAL AGGREGATE SUMMARY ---
    if telemetry_logs:
        df_telemetry = pd.DataFrame(telemetry_logs)
        avg_latency = df_telemetry["Latency (s)"].mean()
        avg_speed = df_telemetry["Throughput (tk/s)"].mean()
        
        print("\n" + "="*35 + " 📊 SERVERLESS TELEMETRY MATRIX " + "="*35)
        print(f"Infrastructure Architecture:  Fireworks AI Serverless Gateway ({CUSTOM_MODEL_ID})")
        print(f"Average Roundtrip Latency:    {avg_latency:.3f} seconds / frame")
        print(f"Average Token Throughput:     {avg_speed:.2f} tokens / second")
        print("="*102 + "\n")
        
    print(f"✅ Pipeline Complete! Output logs securely updated in the workspace root.")
